...
def process_bc_dataset(dataset: dict, cost_limit: float, gamma: float, bc_mode: str, lower_percentile: float = 50.0, upper_percentile: float = 100.0):
    """
    Processes a given dataset for behavior cloning and its variants.
    ...
    Args:
        ...
        bc_mode (str): The behavior cloning mode to use. Can be one of:
            ...
            - "expert": Only trajectories with cost below the cost limit and whose rewards are within a given percentile range are used.
        lower_percentile (float): The lower bound percentile for reward selection (used only in expert mode).
        upper_percentile (float): The upper bound percentile for reward selection (used only in expert mode).
    ...
    if bc_mode == "all" or bc_mode == "multi-task":
        ...
        # expert trajectories: safe trajectories whose rewards fall within a given percentile range.
        safe_mask = dataset["cost_returns"] <= cost_limit
        if np.sum(safe_mask) == 0:
            print("No safe trajectories found for expert mode.")
        else:
            safe_rewards = dataset["rew_returns"][safe_mask]
            lower_bound = np.percentile(safe_rewards, lower_percentile)
            upper_bound = np.percentile(safe_rewards, upper_percentile)
            expert_mask = np.logical_and(dataset["rew_returns"] >= lower_bound,
                                         dataset["rew_returns"] <= upper_bound)
            selected_transition[expert_mask] = 1
    else:
